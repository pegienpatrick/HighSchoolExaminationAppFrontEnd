<template>
  <v-app>
    <v-container>
      ...
      <VCol>
        <VBtn
              :to="`/viewExam?examinationId=${examinationId}&stage=${stage.data}`"
              text="Exam Home"
            ...
            <VCol>
              <VBtn
              :to="`/marksSheet?exam=${examinationId}&stage=${stage.data}`"
              text="view Marks"
              />
            </VCol>
            <VCol>
              ...
              :to="`/viewMeritList?exam=${examinationId}&stage=${stage.data}`"
              />
            </VCol>
            ...
            <VCol>
              <VBtn
              text="Reports"
              :to="`/viewExamReports?examinationId=${examinationId}&stage=${stage.data}`"
              />
            </VCol>
      ...
      <v-row>
        ...
        <v-col cols="12">
          <v-card elevation="3">
            <v-card-title class="text-h6 font-weight-bold">Overall Exam Analysis</v-card-title>
            <v-card-text>
              <VCol cols="12">
  <VCard title="Exam Analysis">
...
export default {
  setup() {
    ...
    const loadPdf = () => {
      ...
      const serverUrl = apiUrl + `/api/v1/meritList/viewAnalysis/${examinationId.value}/${stage.value.data}?stream=${stage.value.stream}`;
